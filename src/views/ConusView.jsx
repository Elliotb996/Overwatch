...
export function ConusView({ auth }) {
  ...
  }).filter(b => b.total > 0 || true) // show all bases
    ...
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ width: 360, borderRight: '1px solid #1e2c3a', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #1e2c3a', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ ...Z, fontSize: 9, color: '#2e4050', marginLeft: 'auto' }}>
        ...
        <div style={{ padding: '6px 12px', borderBottom: '1px solid #1e2c3a', display: 'flex', gap: 4 }}>
          {['socom','total','active'].map(k => (
            <button key={k} onClick={() => setSortBy(k)}
              style={{
                ...Z, fontSize: 8, letterSpacing: 2, padding: '3px 8px', cursor: 'pointer', border: '1px solid',
      ...
      {/* Right — selected base detail */}
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {selectedData ? (
          <BaseDetail base={selectedData} />
        ) : (
          <EmptyState count={flights.length} byBase={byBase} />
        )}
      </div>
    </div>
  )
    ...
    <div onClick={onClick}
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
          ...
          {surge && <span style={{ ...Z, fontSize: 8, letterSpacing: 2, color: '#e8d040', padding: '1px 4px', border: '1px solid rgba(232,208,64,.3)', background: 'rgba(232,208,64,.06)' }}>SURGE</span>}
        </div>
...
function BaseDetail({ base }) {
  ...
  const destBreakdown = Object.entries(base.dests || {})
    .sort((a, b) => b[1] - a[1])

  // MC flag breakdown
  const flagBreakdown = base.flights.reduce((acc, f) => {
    acc[f.mc_flag] = (acc[f.mc_flag] || 0) + 1
    return acc
  }, {})
  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      <div style={{ padding: '20px 24px', borderBottom: '1px solid #1e2c3a' }}>
        ...
        <div style={{ fontSize: 13, color: '#4a6070', marginBottom: 12 }}>{base.name}</div>
        <div style={{ fontSize: 11, color: '#50a0e8' }}>{base.unit}</div>

        {/* Stats row */}
        <div style={{ display: 'flex', gap: 0, marginTop: 16 }}>
          {[
            { label: 'TOTAL',   value: base.total,  color: '#4a6070' },
            { label: 'ACTIVE',  value: base.active, color: '#39e0a0' },
            { label: 'SOF',     value: base.socom,  color: '#a060e8' },
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #1e2c3a', padding: '0 16px' }}>
        {['flights','summary'].map(t => (
          <button key={t} onClick={() => setTab(t)}
            style={{
              ...Z, fontSize: 9, letterSpacing: 2, padding: '8px 12px',
      ...
      {tab === 'summary' ? (
        <div style={{ padding: 20 }}>
          <Section title="DESTINATION BREAKDOWN">
            {destBreakdown.map(([dest, count]) => (
              <BarRow key={dest} label={dest} value={count} max={destBreakdown[0]?.[1] || 1} color="#39e0a0" />
            ))}
          </Section>
          <Section title="MC FLAG BREAKDOWN">
            {Object.entries(flagBreakdown).map(([flag, count]) => (
              <BarRow key={flag} label={flag.toUpperCase()} value={count} max={base.total || 1}
                color={MC_FLAG_COLORS[flag]?.text || '#4a6070'} />
            ))}
          </Section>
        </div>
      ) : (
...
function Section({ title, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ ...Z, fontSize: 9, letterSpacing: 3, color: '#2e4050', marginBottom: 10 }}>{title}</div>
      {children}
    </div>
  )
}
...
function BarRow({ label, value, max, color }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
      <div style={{ ...Z, fontSize: 10, color: '#b8ccd8', width: 80 }}>{label}</div>
      ...
      <div style={{ ...Z, fontSize: 10, color: '#4a6070', width: 20, textAlign: 'right' }}>{value}</div>
    </div>
  )
}
...
function FlightTable({ flights }) {
  if (!flights.length) return (
    <div style={{ padding: 20, ...Z, fontSize: 10, color: '#2e4050' }}>NO RECORDS</div>
  )
  return (
            ...
            return (
              <tr key={f.id} style={{ borderBottom: '1px solid rgba(30,44,58,.4)' }}>
                ...
                <td style={{ padding: '6px 10px' }}>
                  <span style={{ ...col, padding: '1px 5px', fontSize: 8 }}>{(f.mc_flag||'amc').toUpperCase()}</span>
                </td>
...
function EmptyState({ count, byBase }) {
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ ...Z, fontSize: 10, letterSpacing: 3, color: '#2e4050', marginBottom: 24 }}>
        SELECT A BASE TO VIEW DETAIL
      </div>
      <div style={{ ...Z, fontSize: 9, letterSpacing: 2, color: '#2e4050', marginBottom: 12 }}>
        TOP SOF ACTIVITY
      </div>
